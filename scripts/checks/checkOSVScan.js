...
exports.run = async () => {
  ...
  try {
    execSync('npx osv-scanner --lockfile=package-lock.json --format=json > output/osv.json', { stdio: 'inherit' });
    ...
    parsed.results?.forEach(pkg => {
      pkg.vulnerabilities?.forEach(vuln => {
        findings.push({
          ...
          severity: vuln.severity === 'CRITICAL' || vuln.severity === 'HIGH' ? 3 : 2,
    ...
    findings.push({
      type: 'OSV',
      message: 'OSV scan failed or no lockfile',
      file: 'package-lock.json',
      details: err.message,
      severity: 1,
      doc: 'https://osv.dev'
